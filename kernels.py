    ...
    for row in range(threadId_row, data_d.shape[0], stride_x):
        for col in range(threadId_col, data_d.shape[0], stride_y):
            if col > row:
                ...
                linear_cost_table[k] = \
                    round(hypot(data_d[row, 2] - data_d[col, 2],
                          data_d[row, 3] - data_d[col, 3]))
            ...
            if missing_d[row, col] != 0:
                for j in range(2, pop.shape[1]-1):
                    cuda.atomic.compare_and_swap(
                        pop[row, j:], r_flag, missing_d[row, col])
    ...
    for row in range(threadId_row, pop.shape[0], stride_x):
        if threadId_col == 15:
            ...
            while pop[row, i] != r_flag:
                ...
                if pop[row, i] != 1:
                    # index starts from 0 while individuals start from 1
                    reqcap += data_d[pop[row, i]-1, 1]
            ...
            if col+2 < pop.shape[1]:
                ...
                if pop[row, col] == 1 and pop[row, col+1] != 1 and pop[row, col+2] != 1:
                    ...
                    while pop[row, col+route_length] != 1 and col+route_length < pop.shape[1]:
                        auxiliary_arr[row, col +
                                      route_length] = pop[row, col+route_length]
                        route_length += 1
                    ...
                    for idx in range(0, route_length):
                        i = min(auxiliary_arr[row, col+idx]-1,
                                auxiliary_arr[row, col+idx+1]-1)
                        j = max(auxiliary_arr[row, col+idx]-1,
                                auxiliary_arr[row, col+idx+1]-1)
                    ...
                    while improved:
                        ...
                        for idx_i in range(1, route_length-1):
                            ...
                            auxiliary_arr[row, col+idx_i], auxiliary_arr[row, col+idx_i+1] = \
                                auxiliary_arr[row, col+idx_i +
                                              1], auxiliary_arr[row, col+idx_i]

                            # check the cost of the arrangement:
                            for idx_j in range(0, route_length):
                                i = min(
                                    auxiliary_arr[row, col+idx_j]-1, auxiliary_arr[row, col+idx_j+1]-1)
                                j = max(
                                    auxiliary_arr[row, col+idx_j]-1, auxiliary_arr[row, col+idx_j+1]-1)
                            ...
                            if total_cost < min_cost:
                                ...
                                auxiliary_arr[row, col+idx_i+1], auxiliary_arr[row, col+idx_i] =\
                                    auxiliary_arr[row, col +
                                                  idx_i], auxiliary_arr[row, col+idx_i+1]
    ...
    for row in range(threadId_row, auxiliary_arr.shape[0], stride_x):
        for col in range(threadId_col, auxiliary_arr.shape[1] - 1, stride_y):
            if col > 1 and col < child_d_1.shape[1]-1:
                ...
                # Perform crossover with a probability of 0.6
                if random_arr[row, 0] <= crossover_prob:
                    no_cuts = auxiliary_arr[row, 4]
        ...
        if random_no[row, 0] <= mutation_prob:
            for col in range(threadId_col, pop.shape[1], stride_y):
                ...
                if col >= start and col < start+ceil(length/2):
                    pop[row, col], pop[row, ending -
                                       diff] = pop[row, ending-diff], pop[row, col]
...
def findMissingNodes(blocks, threads_per_block, data_d, pop, auxiliary_arr):
    ...
    findExistingNodes[blocks, threads_per_block](
        data_d.shape[0], pop, auxiliary_arr)
...
def generateCutPoints(blocks, threads_per_block, crossover_points, pop_d, popsize, auxiliary_arr):
    if crossover_points == 1:
        ...
        auxiliary_arr[:, 5] = cp.random.randint(
            (pop_d.shape[1]//4)*2, (pop_d.shape[1]//4)*3, size=popsize, dtype=cp.int32)
    else:
        rng_states = create_xoroshiro128p_states(
            popsize*pop_d.shape[1], seed=random.randint(2, 2*10**5))
...
def elitism(child_d_1, child_d_2, pop_d, popsize):
    ...
    pop_d[floor(0.05*popsize):floor(0.5*popsize),
          :] = child_d_1[0:(floor(0.5*popsize)-floor(0.05*popsize)), :]
    pop_d[floor(0.5 * popsize):-1,
          :] = child_d_2[0:(popsize - floor(0.5 * popsize)-1), :]
...
def broadcastPopulation_DGX_1(GPU_ID, pointers, pop_d):
    ...
    try:
        if GPU_ID == 0:
            ...
            cp.cuda.runtime.memcpyPeer(
                pointers[4], 4, pointers[0], 0, pop_d.nbytes)

            # Copy from GPU 0 >> GPU 1 >> GPU 5
            print('\nBroadcasting solutions to GPU 1')
            cp.cuda.runtime.memcpyPeer(
                pointers[1], 1, pointers[0], 0, pop_d.nbytes)

            print('\nBroadcasting solutions to GPU 5')
            cp.cuda.runtime.memcpyPeer(
                pointers[5], 5, pointers[1], 1, pop_d.nbytes)

            # Copy from GPU 0 >> GPU 2 >> GPU 6
            print('\nBroadcasting solutions to GPU 2')
            cp.cuda.runtime.memcpyPeer(
                pointers[2], 2, pointers[0], 0, pop_d.nbytes)

            print('\nBroadcasting solutions to GPU 6')
            cp.cuda.runtime.memcpyPeer(
                pointers[6], 6, pointers[2], 2, pop_d.nbytes)

            # Copy from GPU 0 >> GPU 3 >> GPU 7
            print('\nBroadcasting solutions to GPU 3')
            cp.cuda.runtime.memcpyPeer(
                pointers[3], 3, pointers[0], 0, pop_d.nbytes)
